...
### Before: 処理前のスプレッドシート
ヘッダーだけの空のスプレッドシート。これから自動でデータが投入されます。

![Before](./01_before.png)

### 実行: プログラムで自動処理
`python main.py` を1回実行するだけで、8枚のレシート画像を一括処理。OCR → 日付・店名・金額の抽出 → 科目の自動分類 → スプレッドシート転記まで全自動。

![実行結果](./02_execution.png)

### After: 処理後のスプレッドシート
わずか数十秒で8件分のデータが整理された状態に。手入力なら10分以上かかる作業が自動化されました。
